...
public class CashReceipt {
    ...
    public CashReceipt(Receipt receipt,double grossTotal,double discount,double remaining,double gst,ArrayList<Orderlist> foodlist){
        MemberControl memberControl = new MemberControl();
        String memberName;
        System.out.print(receipt.getId() + "inside cashreceipt");
        System.out.print(receipt.getOrderid()+ "inside cashreceipt");
        OrderDA od = new OrderDA();
        try{
            File file = new File("C:\\Users\\asus\\Desktop\\jhCode\\Cashreceipt.txt","UTF-8");
            
            file.delete();
            FileWriter fw = new FileWriter("Cashreceipt.txt");
